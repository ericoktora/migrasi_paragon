...
cursor_awal.execute("SELECT * FROM duplo_ori_detail")
...
query = """
    INSERT INTO duplo_ori_detail (
        id_duplo_detail, id_duplo, kd_lis, hasil, satuan, nnormal, flag, date_run, created_at, updated_at
    ) VALUES (
        %(id_duplo_detail)s, %(id_duplo)s, %(kd_lis)s, %(hasil)s, %(satuan)s, %(nnormal)s, %(flag)s, %(created_at)s, %(created_at)s, %(updated_at)s
    )
"""
...
print("✅ Selesai memindahkan data duplo_ori_detail.")