...
cursor_awal.execute("SELECT * FROM duplo_ori")
data_awal = cursor_awal.fetchall()

# Query insert
query = """
    INSERT INTO duplo_ori (
        id_duplo, kode, alat, date_tr, date_run, created_at, updated_at
    ) VALUES (
        %(id_duplo)s, %(kode)s, %(alat)s, %(date_tr)s, %(date_run)s, %(created_at)s, %(updated_at)s
    )
"""
...
print("Selesai memindahkan data duplo_ori.")