...
cursor_awal.execute("SELECT * FROM kritis")
...
for row in data_awal:
    try:
        ...
        query = """
            INSERT INTO kritis (
                id, id_kode_lab, `case`, created_at, updated_at
            ) VALUES (
                %(id)s, %(id_kode_lab)s, %(case)s, %(created_at)s, %(updated_at)s
            )
        """
...
db_tujuan.commit()
print("Selesai memindahkan data kritis.")
