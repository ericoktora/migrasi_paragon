...
cursor_awal.execute("SELECT * FROM konten_catatan")
...
for row in data_awal:
    try:
        ...
        query = """
            INSERT INTO konten_catatan (
                id, id_kategori, catatan, created_at, updated_at
            ) VALUES (
                %(id)s, %(id_kategori)s, %(catatan)s, %(created_at)s, %(updated_at)s
            )
        """
...
db_tujuan.commit()
print("Selesai memindahkan data konten_catatan.")
